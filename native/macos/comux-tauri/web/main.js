...
(function () {
  ...
  function loadSettings() {
    ...
    try {
      ...
      return {
        ...
        maxBrowserTabsPerProject: clampInt(
          saved.maxBrowserTabsPerProject,
          defaults.maxBrowserTabsPerProject,
          1,
          HARD_MAX_BROWSER_TABS_PER_PROJECT
        ),
      };
    } catch (_) {
      return defaults;
  ...
  function saveSettings() {
    ...
    settings.maxBrowserTabsPerProject = clampInt(
      settings.maxBrowserTabsPerProject,
      10,
      1,
      HARD_MAX_BROWSER_TABS_PER_PROJECT
    );
  ...
  function persistableProject(project) {
    return {
      ...
      name: project.name,
      root: project.root,
      layout: ensureProjectLayout(project) || { mode: "terminal", side: "right", splitFrac: 0.6 },
      browser: ensureBrowserModel(project) || { tabs: [], activeTabId: null },
      lastActiveThreadId: project.lastActiveThreadId || null,
    };
  }
  ...
  function readSavedWorkspace() {
    try {
      var saved = JSON.parse(localStorage.getItem(WORKSPACE_STATE_KEY) || "null");
      if (!saved || !Array.isArray(saved.projects)) return null;
      return saved;
    } catch (_) {
      return null;
    }
  ...
  function sanitizeSavedProject(saved) {
    ...
    var project = {
      ...
      layout: {
        ...
        splitFrac: clampInt(saved.layout && saved.layout.splitFrac ? Math.round(saved.layout.splitFrac * 1000) : 600, 600, 200, 850) / 1000,
      },
      browser: { tabs: [], activeTabId: null },
      lastActiveThreadId: saved.lastActiveThreadId || null,
    };
    ...
    if (Array.isArray(savedBrowser.tabs)) {
      project.browser.tabs = savedBrowser.tabs.slice(0, HARD_MAX_BROWSER_TABS_PER_PROJECT).map(function (tab) {
        ...
        return {
          ...
          url: url,
          title: tab.title || tabTitle(url),
          history: history,
          historyIndex: clampInt(tab.historyIndex, history.length ? history.length - 1 : -1, -1, Math.max(-1, history.length - 1)),
          created: !!tab.created && url !== "about:blank",
          loading: false,
        };
      });
    }
  ...
  function setProjectStatus(project, level) {
    ...
    setStatus(project ? project.name : "ready", level || "ok");
  }
  ...
  function currentSplitFrac() {
    return parseFloat(detail.style.getPropertyValue("--split-frac")) || 0.6;
  }
  function ensureProjectLayout(project) {
    if (!project) return null;
    if (!project.layout) {
      project.layout = { mode: "terminal", side: "right", splitFrac: 0.6 };
    }
  ...
  function createThread(opts) {
    ...
    var projectPaneCount = project
      ? state.threads.filter(function (t) { return t.projectId === project.id; }).length
      : state.threads.length;
    var thread = {
      id: id,
      projectId: project ? project.id : null,
      name: opts.name || "pane " + (projectPaneCount + 1),
  ...
  // ============================================================
  // 7. Focused project tab strip
  // ============================================================

  function refreshSidebar() { refreshTabs(); }
  ...
  // Tabs are projects. The active project's panes remain scoped in the focused
  // terminal host/comux pane rather than duplicated as another top tab row.
  function refreshTabs() {
    ...
    if (state.projects.length === 0) {
      ...
      empty.textContent = "no project open — click + to add one";
    ...
    state.projects.forEach(function (project) {
      ...
      tab.className = "tab" + statusClass + (isActive ? " active" : "");
      tab.innerHTML =
        '<span class="dot"></span>' +
  ...
  function updateSettingsCommand(rest) {
    var parts = String(rest || "").trim().split(/\s+/).filter(Boolean);
    if (parts.length === 0) {
      writeToActive(
        "\r\n\x1b[36m[settings]\x1b[0m projects=" + settings.maxProjects +
        " (hard max " + HARD_MAX_PROJECTS + "), browser-tabs=" + settings.maxBrowserTabsPerProject +
        " (hard max " + HARD_MAX_BROWSER_TABS_PER_PROJECT + ")\r\n" +
        "Try /settings projects 8 or /settings browser-tabs 6\r\n"
      );
      return;
    }
    var key = parts[0];
    var value = parts[1];
    if (key === "projects") {
      settings.maxProjects = clampInt(value, settings.maxProjects, 1, HARD_MAX_PROJECTS);
      saveSettings();
      setStatus("project cap set to " + settings.maxProjects, "ok");
      writeToActive("\r\n\x1b[36m[settings]\x1b[0m project cap: " + settings.maxProjects + "/" + HARD_MAX_PROJECTS + "\r\n");
      return;
    }
    if (key === "browser-tabs" || key === "tabs") {
      settings.maxBrowserTabsPerProject = clampInt(value, settings.maxBrowserTabsPerProject, 1, HARD_MAX_BROWSER_TABS_PER_PROJECT);
      saveSettings();
      renderBrowserTabs();
      setStatus("browser tab cap set to " + settings.maxBrowserTabsPerProject + "/project", "ok");
      writeToActive("\r\n\x1b[36m[settings]\x1b[0m browser tabs per project: " + settings.maxBrowserTabsPerProject + "/" + HARD_MAX_BROWSER_TABS_PER_PROJECT + "\r\n");
      return;
    }
    writeToActive("\r\n\x1b[33m[settings]\x1b[0m unknown key. Use projects or browser-tabs.\r\n");
  }
  ...
  var commands = [
    ...
    {
      cmd: "/preview",
      desc: "Load a URL in this project's browser: /preview localhost:5173",
      run: function (rest) {
        if (!rest) { applyLayout("split"); syncProjectBrowser(); return; }
        applyLayout("split");
    ...
    {
      cmd: "/browser-tab",
      desc: "Open a new browser tab for this project: /browser-tab https://example.com",
      run: function (rest) {
        var tab = createBrowserTab(activeProject(), rest || "about:blank", true);
        if (tab && rest) navigateBrowser(rest, { tabId: tab.id, replace: true });
        else { applyLayout("split"); renderBrowserTabs(); syncProjectBrowser(); }
      },
    },
    {
      cmd: "/split",
    ...
    {
      cmd: "/settings",
      desc: "Set caps: /settings projects 8 or /settings browser-tabs 6 (hard max 10)",
      run: function (rest) { updateSettingsCommand(rest); },
    },
    {
      cmd: "/run",
  ...
  function builtinPaletteEntries() {
    return commands.map(function (c) {
      return { cmd: c.cmd, desc: c.desc, badge: "comux", kind: "builtin" };
    });
  }
  ...
  function paletteCorpus() {
    return builtinPaletteEntries().concat(agentSkillPaletteEntries());
  }
  ...
  function renderPalette() {
    ...
    paletteFiltered.forEach(function (c, idx) {
      ...
      div.addEventListener("click", function () {
        // Built-in commands take an optional argument, so we leave a trailing
        // space; agent-harness skills are typically invoked bare, so we
        // submit on click.
        if (c.kind === "agent") {
          runCommand(c.cmd);
          commandInput.value = "";
  ...
  commandInput.addEventListener("keydown", function (e) {
    if (paletteVisible) {
      ...
      if (e.key === "Tab") {
        ...
        // Agent skills don't take args — Tab autocompletes without a trailing
        // space so Enter immediately fires the harness command.
        commandInput.value = pick.cmd + (pick.kind === "agent" ? "" : " ");
        hidePalette();
        e.preventDefault();
        return;
      }
  ...
  function ensureBrowserModel(project) {
    if (!project) return null;
    if (!project.browser) {
      project.browser = { tabs: [], activeTabId: null };
    }
  ...
  function createBrowserTab(project, url, activate) {
    ...
    if (browser.tabs.length >= maxTabs) {
      setStatus("browser tab limit reached (" + maxTabs + "/project)", "warn");
      return null;
    }
    var normalised = url && url !== "about:blank" ? normaliseUrl(url) : "about:blank";
    var tab = {
      id: makeBrowserTabId(),
      url: normalised || "about:blank",
      title: tabTitle(normalised),
      history: normalised && normalised !== "about:blank" ? [normalised] : [],
      historyIndex: normalised && normalised !== "about:blank" ? 0 : -1,
      created: false,
      loading: false,
    };
    browser.tabs.push(tab);
    if (activate || !browser.activeTabId) browser.activeTabId = tab.id;
    renderBrowserTabs();
    saveWorkspaceSoon();
    return tab;
  }
  ...
  function closeBrowserTab(project, tabId) {
    ...
    var browser = ensureBrowserModel(project);
    if (!browser) return;
    var idx = browser.tabs.findIndex(function (t) { return t.id === tabId; });
    if (idx < 0) return;
  ...
  function openBlankBrowserTab() {
    createBrowserTab(activeProject(), "about:blank", true);
    applyLayout("split");
    syncProjectBrowser();
  ...
  function appendBrowserTabAddButton() {
    ...
    add.textContent = "+";
    add.title = "New browser tab for this project";
    add.addEventListener("click", openBlankBrowserTab);
    browserTabStrip.appendChild(add);
  }
  ...
  function renderBrowserTabs() {
    ...
    var project = activeProject();
    var browser = ensureBrowserModel(project);
    browserTabStrip.innerHTML = "";
    if (!browser || browser.tabs.length === 0) {
      var empty = document.createElement("span");
      empty.className = "browser-tab-empty";
      empty.textContent = "project browser";
      browserTabStrip.appendChild(empty);
      appendBrowserTabAddButton();
      syncUrlInput();
      return;
    }
    browser.tabs.forEach(function (tab) {
      ...
      btn.addEventListener("click", function (event) {
        if (event.target && event.target.classList.contains("browser-tab-close")) {
          ...
          activateBrowserTab(project, tab.id);
        }
      });
  ...
  function updateBrowserControls() {
    ...
    var external = document.getElementById("open-external");
    if (back) back.disabled = !(tab && tab.historyIndex > 0);
    if (forward) forward.disabled = !(tab && tab.historyIndex < tab.history.length - 1);
    if (reload) {
      reload.disabled = !(tab && tab.created);
      reload.classList.toggle("loading", !!(tab && tab.loading));
    }
  ...
  function visibleBrowserBounds() {
    ...
    if (detail.dataset.layout === "terminal") return null;
  ...
  function syncBrowserBounds() {
    ...
    var b = visibleBrowserBounds();
    if (!b || !tab || !tab.created) {
      invoke("browser_hide_all_except", { label: null }).catch(function () {});
      return;
    }
  ...
  function navigateBrowser(rawUrl, opts) {
    ...
    if (!project) return;
    var browser = ensureBrowserModel(project);
    var tab = opts.tabId ? browser.tabs.find(function (t) { return t.id === opts.tabId; }) : currentBrowserTab(project);
    if (!tab) tab = createBrowserTab(project, rawUrl || "about:blank", true);
    if (!tab) return;
    browser.activeTabId = tab.id;
    var b = visibleBrowserBounds();
    if (!b) {
      applyLayout("split");
      b = visibleBrowserBounds();
      if (!b) return;
    }
    var normalised = normaliseUrl(rawUrl);
    if (!normalised) return;
    tab.loading = true;
    tab.title = tabTitle(normalised);
    renderBrowserTabs();
    updateBrowserControls();
    var label = browserLabelForTab(project, tab);
  ...
  function normaliseUrl(value) {
    ...
    if (trimmed.indexOf("://") === -1) {
      var local = /^(localhost|127\.0\.0\.1|0\.0\.0\.0|192\.168\.|10\.|::1)/i.test(trimmed);
  ...
  urlInput.addEventListener("keydown", function (e) {
    if (e.key === "Enter") navigateBrowser(urlInput.value);
  });
  document.getElementById("reload").addEventListener("click", function () {
    ...
    if (tab && tab.created) {
      ...
      renderBrowserTabs();
      updateBrowserControls();
      invoke("browser_reload", { label: browserLabelForTab(project, tab) })
        .catch(function () {})
        .finally(function () {
          setTimeout(function () {
            tab.loading = false;
  ...
  document.getElementById("back").addEventListener("click", function () {
    ...
    if (tab && tab.historyIndex > 0) {
      tab.historyIndex -= 1;
      navigateBrowser(tab.history[tab.historyIndex], { fromHistory: true });
    }
  });
  ...
  document.getElementById("open-external").addEventListener("click", function () {
    var tab = currentBrowserTab();
    if (tab && tab.url && tab.url !== "about:blank" && openUrl) {
      openUrl(tab.url).catch(function () {});
    }
  });

  if (typeof ResizeObserver === "function") {
    var ro = new ResizeObserver(function () { syncBrowserBounds(); });
    ro.observe(preview);
    ro.observe(detail);
  }
  ...
  function browserHasKeyboardFocus(target) {
    if (currentLayout() === "browser") return true;
    if (!target) return false;
    return !!(
      target === urlInput ||
      (browserTabStrip && browserTabStrip.contains(target)) ||
      (target.closest && target.closest(".browser-bar"))
    );
  }
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (!meta) return;
    // ⌘T is contextual: browser tab from browser chrome, terminal pane from terminal/comux.
    if (e.key === "t") {
      if (browserHasKeyboardFocus(e.target)) openBlankBrowserTab();
      else runCommand("/new-thread");
      e.preventDefault(); return;
    }
    // ⌘O opens a new project (folder picker → addProject → comux).
    if (e.key === "o") { openProjectPicker(); e.preventDefault(); return; }
    // ⌘W closes the active pane; ⇧⌘W closes the active project.
    if (e.key === "w") {
      if (e.shiftKey && state.activeProjectId) closeProject(state.activeProjectId);
      else if (state.activeThreadId) closeThread(state.activeThreadId);
      e.preventDefault(); return;
    }
    ...
    // ⌘[ / ⌘] cycle between project workspaces.
    if (e.key === "[") { switchTab(-1); e.preventDefault(); return; }
    if (e.key === "]") { switchTab(+1); e.preventDefault(); return; }
    // ⌘1-9 activates the Nth project workspace.
  ...
  function addProject(rootPath) {
    ...
    if (state.projects.length >= settings.maxProjects) {
      setStatus("project limit reached (" + settings.maxProjects + "/" + HARD_MAX_PROJECTS + ")", "warn");
      return null;
    }
  ...
  async function openProjectPicker() {
    ...
    try {
      ...
      if (project) {
        ensureProjectComux(project);
        setStatus("opened " + project.name, "ok");
      }
    } catch (err) {
  ...
  function ensureProjectComux(project) {
    ...
    var existing = state.threads.find(function (t) {
      return t.projectId === project.id && t.kind === "comux" && t.status !== "exited";
    });
    if (existing) {
  ...
  invoke("app_environment")
    .then(function (env) {
      ...
      if (saved && saved.projects.length) {
        ...
        state.activeProjectId = saved.activeProjectId && state.projects.some(function (p) { return p.id === saved.activeProjectId; })
      ...
      if (!project) {
        project = addProject(bootRoot);
      }

      if (project) {
        ensureProjectComux(project);
        project.browser.tabs.forEach(function (tab) {
          if (tab.created && tab.url && tab.url !== "about:blank") {
            // Recreate persisted tabs lazily enough to restore the visible page
            // without stealing focus from the comux pane. Only the active tab is
            // shown; inactive tab webviews stay hidden until selected.
            navigateBrowser(tab.url, { tabId: tab.id, replace: true });
          }
        });
        restoreProjectLayout(project);
      }
