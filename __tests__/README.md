...
This directory contains comprehensive unit tests for all vmux action implementations. These tests verify that actions work correctly in isolation by mocking external dependencies.
...
│   └── mockPanes.ts       # VmuxPane fixtures
...
- Create mock ActionContext and VmuxPane fixtures
...
it('should jump to pane successfully', async () => {
  ...
  expect(result.targetPaneId).toBe('vmux-1');
});
```
...
- Following pattern in `vmux.e2e.create-pane.test.ts`
...
**mockPanes.ts** - Creates mock VmuxPane objects:
```typescript
