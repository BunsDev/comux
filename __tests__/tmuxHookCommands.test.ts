...
describe('tmuxHookCommands', () => {
  it('builds pane-exited hook with recovery + SIGUSR2 notification', () => {
    ...
    expect(command).toContain('# comux-hook');
  });
  ...
  it('encodes shell-sensitive session names safely', () => {
    ...
    expect(command).toContain(`COMUX_RECOVERY_SESSION_B64=${encodedSession}`);
  });
  ...
  it('builds pane-focus hook without shelling out to tmux', () => {
    ...
    expect(command).toContain('if-shell -F "#{!=:#{@comux_active_border_style},}"');
    expect(command).toContain('set-option -F -t \\"my\\"session\\$\\`x\\\\y\\" pane-active-border-style');
    expect(command).toContain('#{@comux_active_border_style}');
    expect(command).toContain('run-shell -b "kill -USR2 99 2>/dev/null || true # comux-hook"');
