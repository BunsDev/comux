...
describe('tmuxHookCommands', () => {
  it('builds pane-exited hook with recovery + SIGUSR2 notification', () => {
    const command = buildPaneExitedHookCommandForSession(4321, 'vmux-test');

    expect(command).toContain('VMUX_RECOVERY_SESSION_B64=dm11eC10ZXN0');
    expect(command).toContain('VMUX_RECOVERY_EXITED_PANE=#{hook_pane}');
    expect(command).toContain('controlPaneRecovery.js');
    expect(command).toContain('kill -USR2 4321');
    expect(command).toContain('# vmux-hook');
  });
  ...
  it('encodes shell-sensitive session names safely', () => {
    ...
    expect(command).toContain(`VMUX_RECOVERY_SESSION_B64=${encodedSession}`);
  });
  ...
  it('builds pane-focus hook without shelling out to tmux', () => {
    ...
    expect(command).toContain('if-shell -F "#{!=:#{@vmux_active_border_style},}"');
    expect(command).toContain('set-option -F -t \\"my\\"session\\$\\`x\\\\y\\" pane-active-border-style');
    expect(command).toContain('#{@vmux_active_border_style}');
    expect(command).toContain('run-shell -b "kill -USR2 99 2>/dev/null || true # vmux-hook"');
