...
describe.sequential('vmux e2e: input wrapping interactions', () => {
  it.runIf(canRun)('wraps full word at the moment overflow occurs (on screen)', async () => {
    const server = `vmux-e2e-wrap-${Date.now()}`;
    const session = `vmux-e2e-wrap`;
    ...
    try {
      ...
      // Wait longer for vmux to fully initialize
      await sleep(1000);
