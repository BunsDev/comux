...
vi.mock('../src/utils/git.js', () => ({
  ...
  getPaneBranchName: (pane: VmuxPane) => pane.branchName || pane.slug,
}));
...
describe('merge target resolution', () => {
    ...
    const chain = createMergeTargetChain(
      {
        ...
        prompt: 'parent',
        paneId: '%1',
        worktreePath: '/repo/.vmux/worktrees/feature-parent',
      },
      '/repo'
    );
  ...
  it('merges top-level worktrees back into the project root checkout', () => {
    ...
    const resolution = resolveMergeTarget({
      ...
      prompt: 'test',
      paneId: '%1',
      worktreePath: '/repo/.vmux/worktrees/feature-a',
    });
  ...
  it('uses the parent worktree when it still exists and has not been merged upstream', () => {
    mocked.existsSync.mockImplementation((value: string) => (
      value === '/repo/.vmux/worktrees/feature-parent'
      || value === '/repo'
    ));
    mocked.getCurrentBranch.mockImplementation((value: string) => (
      value === '/repo/.vmux/worktrees/feature-parent' ? 'feature-parent' : 'main'
    ));
    ...
    const resolution = resolveMergeTarget({
      ...
      prompt: 'test',
      paneId: '%2',
      worktreePath: '/repo/.vmux/worktrees/feature-child',
      mergeTargetChain: [
        {
          slug: 'feature-parent',
          branchName: 'feature-parent',
    ...
    expect(resolution).toMatchObject({
      targetRepoPath: '/repo/.vmux/worktrees/feature-parent',
