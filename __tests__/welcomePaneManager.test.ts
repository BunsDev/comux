...
describe('welcomePaneManager', () => {
  ...
  beforeEach(() => {
    ...
    tempProjectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'vmux-welcome-pane-'));
    const vmuxDir = path.join(tempProjectRoot, '.vmux');
    fs.mkdirSync(vmuxDir, { recursive: true });
    configPath = path.join(vmuxDir, 'vmux.config.json');
  ...
  it('destroys the welcome pane when vmux panes become visible again', async () => {
