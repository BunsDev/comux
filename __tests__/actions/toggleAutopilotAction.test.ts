...
describe('toggleAutopilotAction', () => {
  ...
  it('should enable autopilot when currently disabled', async () => {
    const mockPane = createMockPane({ id: 'vmux-1', slug: 'test', autopilot: false });
  ...
  it('should disable autopilot when currently enabled', async () => {
    const mockPane = createMockPane({ id: 'vmux-1', slug: 'test', autopilot: true });
    const mockContext = createMockContext([mockPane]);
    const savePanesSpy = vi.spyOn(mockContext, 'savePanes');
  ...
  it('should only update the specified pane in multi-pane context', async () => {
    const pane1 = createMockPane({ id: 'vmux-1', autopilot: false });
    const pane2 = createMockPane({ id: 'vmux-2', autopilot: true });
    const pane3 = createMockPane({ id: 'vmux-3', autopilot: false });
    ...
    expect(savePanesSpy).toHaveBeenCalledWith([
      expect.objectContaining({ id: 'vmux-1', autopilot: false }),
      expect.objectContaining({ id: 'vmux-2', autopilot: false }), // Toggled
      expect.objectContaining({ id: 'vmux-3', autopilot: false }),
    ]);
  });
