...
describe('getPaneBranchName', () => {
  it('returns branchName when set', () => {
    const pane = {
      id: 'vmux-1', slug: 'fix-auth', branchName: 'feat/fix-auth',
  ...
  it('falls back to slug when branchName is undefined', () => {
    const pane = {
      id: 'vmux-1', slug: 'fix-auth', branchName: undefined,
      prompt: 'test', paneId: '%1',
    };
    expect(getPaneBranchName(pane)).toBe('fix-auth');
  });
...
describe('slug and branchName separation', () => {
  ...
  it('worktree path uses slug, not branchName', () => {
    ...
    const slug = 'fix-auth';
    const worktreePath = `${projectRoot}/.vmux/worktrees/${slug}`;

    expect(worktreePath).toBe('/home/user/project/.vmux/worktrees/fix-auth');
...
describe('baseBranch in worktree creation command', () => {
  it('produces correct command with baseBranch as start-point', () => {
    const worktreePath = '/project/.vmux/worktrees/fix-auth';
    ...
    expect(cmd).toBe('git worktree add "/project/.vmux/worktrees/fix-auth" -b "feat/fix-auth" "main"');
  });
  ...
  it('produces correct command without baseBranch (uses HEAD)', () => {
    ...
    expect(cmd).toBe('git worktree add "/project/.vmux/worktrees/fix-auth" -b "fix-auth"');
  });
  ...
  it('uses existing branch without -b flag when branch exists', () => {
    ...
    expect(cmd).toBe('git worktree add "/project/.vmux/worktrees/fix-auth" "feat/fix-auth"');
  });
});

// ─── VMUX_BRANCH hook env ───
