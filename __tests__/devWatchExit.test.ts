...
const originalDevWatch = process.env.VMUX_DEV_WATCH

afterEach(() => {
  if (originalDevWatch === undefined) {
    delete process.env.VMUX_DEV_WATCH
    return
  }

  process.env.VMUX_DEV_WATCH = originalDevWatch
})
...
describe("shouldUseQuietDevWatchExit", () => {
  ...
  it("returns false outside dev watch mode", () => {
    process.env.VMUX_DEV_WATCH = "false"
