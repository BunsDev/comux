...
const originalDevWatch = process.env.COMUX_DEV_WATCH

afterEach(() => {
  if (originalDevWatch === undefined) {
    delete process.env.COMUX_DEV_WATCH
    return
  }

  process.env.COMUX_DEV_WATCH = originalDevWatch
})
...
describe("shouldUseQuietDevWatchExit", () => {
  ...
  it("returns false outside dev watch mode", () => {
    process.env.COMUX_DEV_WATCH = "false"
