...
const ComuxApp = ({ panesFile, projectName, sessionName, settingsFile, projectRoot, autoUpdater, controlPaneId, }) => {
...
//# sourceMappingURL=ComuxApp.js.map`,
...
class Comux {
    ...
    async init() {
        ...
        if (inTmux &&
            ...
            const shouldAttachToCurrent = await this.promptYesNo(\`Detected active comux session '\${currentTmuxSessionName}'. Add project '\${this.projectName}' to this session?\`, true);
        ...
        if (!inTmux) {
            ...
            try {
                ...
                const sessionOptions = [
                    ...
                    \`set-option -t \${this.sessionName} pane-border-format " #{s|__comux__.*$||:pane_title} "\`,
                    \`select-pane -t \${this.sessionName} -T "comux v\${packageJson.version} - \${this.projectName}"\`,
                ].join(' \\\\; ');
                ...
                let comuxCommand;
                if (isDev) {
                    comuxCommand = \`cd "\${devDirectory}" && pnpm dev:watch\`;
                }
                else {
                ...
                execSync(\`tmux send-keys -t \${this.sessionName} "\${comuxCommand}" Enter\`, { stdio: 'inherit' });
            }
        ...
        const shouldPublishMetadata = !metadataSessionName.startsWith('comux-') || metadataSessionName === this.sessionName;
    ...
    async attachProjectToExistingSession(sessionName) {
        ...
        if (!context) {
            console.log(chalk.yellow(\`Unable to locate config for session '\${sessionName}'. Run comux inside that project once, then try again.\`));
            return false;
        }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=yes">
  <title>Terminal Viewer - comux</title>
