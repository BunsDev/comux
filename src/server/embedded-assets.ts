...
const VmuxApp = ({ panesFile, projectName, sessionName, settingsFile, projectRoot, autoUpdater, controlPaneId, }) => {
...
//# sourceMappingURL=VmuxApp.js.map`,
...
class Vmux {
    ...
    async init() {
        ...
        if (inTmux &&
            ...
            const shouldAttachToCurrent = await this.promptYesNo(\`Detected active vmux session '\${currentTmuxSessionName}'. Add project '\${this.projectName}' to this session?\`, true);
        ...
        if (!inTmux) {
            ...
            try {
                ...
                const sessionOptions = [
                    ...
                    \`set-option -t \${this.sessionName} pane-border-format " #{s|__vmux__.*$||:pane_title} "\`,
                    \`select-pane -t \${this.sessionName} -T "vmux v\${packageJson.version} - \${this.projectName}"\`,
                ].join(' \\\\; ');
                ...
                let vmuxCommand;
                if (isDev) {
                    vmuxCommand = \`cd "\${devDirectory}" && pnpm dev:watch\`;
                }
                else {
                ...
                execSync(\`tmux send-keys -t \${this.sessionName} "\${vmuxCommand}" Enter\`, { stdio: 'inherit' });
            }
        ...
        const shouldPublishMetadata = !metadataSessionName.startsWith('vmux-') || metadataSessionName === this.sessionName;
    ...
    async attachProjectToExistingSession(sessionName) {
        ...
        if (!context) {
            console.log(chalk.yellow(\`Unable to locate config for session '\${sessionName}'. Run vmux inside that project once, then try again.\`));
            return false;
        }
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=yes">
  <title>Terminal Viewer - vmux</title>
