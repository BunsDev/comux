...
const HooksDialog: React.FC<HooksDialogProps> = ({
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={COLORS.accent} paddingX={1} marginTop={1}>
      <Box marginBottom={1}>
        <Text bold color={COLORS.accent}>Hooks Management</Text>
        ...
        return (
          <Box key={hook.name} flexDirection="column">
            <Box>
              ...
              <Text color={hook.active ? COLORS.success : COLORS.muted} dimColor={!isSelected}>
      ...
      <Box marginTop={1} flexDirection="column">
        <Box>
          <Text bold color={COLORS.accent}>Edit hooks using an agent</Text>
        </Box>
        <Box marginLeft={2}>
