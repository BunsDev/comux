...
const CleanTextInput: React.FC<CleanTextInputProps> = ({
  ...
  useInput((input, key) => {
    ...
    if (input && !key.ctrl && !key.meta) {
      // Ignore the first character input if flag is set (prevents 'n' from vmux menu)
  ...
  const renderTextWithTags = (text: string, isInverse: boolean = false): React.ReactNode[] => {
    ...
    return parts.map((part, i) => {
      if (part.match(tagPattern)) {
        ...
        return <Text key={i} color={COLORS.accent} dimColor>{part}</Text>;
      }
      // Regular text
              ...
              <Box>
                {cursorInTag ? (
                  ...
                  <>
                    ...
                    <Text inverse color={COLORS.accent}>{at}</Text>
