#!/usr/bin/env node
/**
 * Logs Popup - Scrollable log viewer for vmux
 *
...
const LogsPopupApp: React.FC<LogsPopupAppProps> = ({ allLogs, stats, panes = [], resultFile }) => {
  ...
  useInput((input, key) => {
    ...
    if (input === 'o') {
      ...
      // Write to temp file
      const tempFile = path.join(os.tmpdir(), `vmux-logs-${Date.now()}.txt`);
      try {
    ...
    return (
      <Box key={log.id} flexDirection="column">
        <Text>
          ...
          <Text dimColor>[{log.source || 'vmux'}]</Text>
          <Text color={color}>
            {' '}
            {log.message}
          </Text>
      ...
      <Box flexDirection="column">
        ...
        <Box borderStyle="single" borderColor={POPUP_CONFIG.borderColor} paddingX={1}>
          <Text dimColor>
            ...
            {copied && <Text color={POPUP_CONFIG.successColor}> • ✓ Copied!</Text>}
          </Text>
        </Box>
      </Box>
    </PopupWrapper>
