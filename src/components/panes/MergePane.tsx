...
export default function MergePane({ pane, onComplete, onCancel, mainBranch }: MergePaneProps) {
  ...
  const performMerge = async () => {
    ...
    // Step 2: Get the main repository path (parent of .comux/worktrees)
    const mainRepoPath = pane.worktreePath?.replace(/\/\.comux\/worktrees\/[^/]+$/, '');
    if (!mainRepoPath) {
  ...
  return (
    <Box flexDirection="column" padding={1}>
        ...
        <Box borderStyle="single" borderColor="gray" flexDirection="column" padding={1} marginBottom={1} height={10}>
          ...
          {commandHistory.slice(-5).map((cmd, i) => (
            <Box key={i} flexDirection="column">
              <Text color={COLORS.accent}>$ {cmd.command}</Text>
              {cmd.output && <Text dimColor>{cmd.output.substring(0, 200)}</Text>}
              {cmd.error && <Text color={COLORS.error}>{cmd.error.substring(0, 200)}</Text>}
            </Box>
          ))}
        </Box>
      )}
      ...
      {showResolutionPrompt && !showAgentPromptInput && (
        <Box borderStyle="double" borderColor={COLORS.warning} flexDirection="column" padding={1}>
          <Text color={COLORS.warning} bold>Merge Conflict Resolution Required</Text>
          <Text>Conflicted files:</Text>
          {conflictFiles.map(file => (
            <Text key={file} color={COLORS.error}>  • {file}</Text>
          ))}
          <Text> </Text>
          <Text>Choose resolution method:</Text>
          <Text color={COLORS.accent}>  (A) Resolve with AI agent</Text>
          <Text color={COLORS.success}>  (M) Resolve manually</Text>
          <Text color={COLORS.muted}>  (C) Cancel merge</Text>
        </Box>
      )}

      {showAgentPromptInput && (
        <Box borderStyle="round" borderColor={COLORS.accent} flexDirection="column" padding={1}>
      ...
      {status === 'success' && (
        <Box borderStyle="double" borderColor={COLORS.success} flexDirection="column" padding={1}>
          <Text color={COLORS.success} bold>✓ Merge completed successfully!</Text>
      ...
      {status === 'error' && (
        <Box borderStyle="double" borderColor={COLORS.error} flexDirection="column" padding={1}>
          <Text color={COLORS.error} bold>✗ Merge failed</Text>
          <Text>{error}</Text>
          <Text> </Text>
