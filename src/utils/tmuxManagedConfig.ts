...
export const COMUX_TMUX_CONFIG_START = '# >>> comux';
export const COMUX_TMUX_CONFIG_END = '# <<< comux';
export const COMUX_TMUX_CONFIG_VERSION = 1;
...
export function buildRecommendedTmuxConfig(theme: TmuxPresetTheme): string {
  ...
  return [
    '# =============================================',
    '# comux recommended tmux config',
    `# Theme: ${theme}`,
    '# Generated by comux onboarding',
    '# =============================================',
    '',
...
export function buildComuxManagedTmuxConfigBlock(theme: TmuxPresetTheme = 'dark'): string {
  return [
    COMUX_TMUX_CONFIG_START,
    '# Managed by comux. Edit outside this block; comux may replace this block.',
    `# Version: ${COMUX_TMUX_CONFIG_VERSION}`,
...
export function hasComuxManagedTmuxConfigBlock(content: string): boolean {
  return content.includes(COMUX_TMUX_CONFIG_START) && content.includes(COMUX_TMUX_CONFIG_END);
}

export function upsertComuxManagedTmuxConfigBlock(
  existingContent: string,
  nextBlock: string
): { content: string; action: ManagedTmuxConfigAction; changed: boolean } {
  const normalizedBlock = nextBlock.endsWith('\n') ? nextBlock : `${nextBlock}\n`;
  const startIndex = existingContent.indexOf(COMUX_TMUX_CONFIG_START);
  const endIndex = existingContent.indexOf(COMUX_TMUX_CONFIG_END);

  if (startIndex !== -1 && endIndex !== -1 && endIndex > startIndex) {
    const replaceEnd = endIndex + COMUX_TMUX_CONFIG_END.length;
...
function buildBackupPath(configPath: string, now: Date): string {
  ...
  return `${configPath}.comux-backup-${timestamp}`;
}

export async function writeComuxManagedTmuxConfig(
  homeDir: string,
  theme: TmuxPresetTheme = 'dark',
  now: Date = new Date()
): Promise<ManagedTmuxConfigWriteResult> {
  const configPath = path.join(homeDir, '.tmux.conf');
  const block = buildComuxManagedTmuxConfigBlock(theme);
  let existingContent = '';
  ...
  const result = upsertComuxManagedTmuxConfigBlock(existingContent, block);
  ...
  return {
    ...
    managedBlockVersion: COMUX_TMUX_CONFIG_VERSION,
  };
}
