...
export function buildMissingGitHubCliMessage(
  ...
  if (!installCommand) {
    return 'GitHub CLI (gh) is required to create pull requests from vmux. Install it from https://github.com/cli/cli#installation and then try again.';
  }

  return `GitHub CLI (gh) is required to create pull requests from vmux. Install it first with:\n${installCommand}`;
}
