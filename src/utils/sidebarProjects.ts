...
const AUTO_SIDEBAR_THEME_ORDER: readonly VmuxThemeName[] = [
  DEFAULT_VMUX_THEME,
  ...VMUX_THEME_NAMES.filter((themeName) => themeName !== DEFAULT_VMUX_THEME),
];
...
function normalizeProjectColorTheme(colorTheme: unknown): VmuxThemeName | undefined {
  return isVmuxThemeName(colorTheme) ? colorTheme : undefined;
}
...
export function setSidebarProjectName(
  projects: SidebarProject[],
  projectRoot: string,
  projectName?: string
): SidebarProject[] {
  const normalizedRoot = normalizeProjectRoot(projectRoot);
  const nextName = projectName?.trim() || path.basename(normalizedRoot) || 'project';
  let found = false;
  let changed = false;
  ...
  const nextProjects = projects.map((project) => {
    ...
    found = true;
    if (project.projectName === nextName) {
      return project;
    }
  ...
  if (!found) {
    changed = true;
    nextProjects.push(buildProjectEntry(normalizedRoot, nextName));
  }
...
export function getSidebarProjectDisplayName(
  projects: SidebarProject[],
  projectRoot: string,
  fallbackName?: string
): string {
  const project = getSidebarProject(projects, projectRoot);
  const fromProject = project?.projectName?.trim();
  if (fromProject) {
    return fromProject;
  }
  ...
  return path.basename(normalizeProjectRoot(projectRoot)) || 'project';
}

export function setSidebarProjectColorTheme(
  projects: SidebarProject[],
  projectRoot: string,
  colorTheme: VmuxThemeName | undefined,
...
export function getSidebarProjectColorThemeSettingValue(
  projects: SidebarProject[],
  projectRoot: string,
  resolveProjectTheme?: (projectRoot: string) => VmuxThemeName | undefined
): string {
  const project = getSidebarProject(projects, projectRoot);
...
export function setSidebarProjectColorThemeSettingValue(
  ...
  if (isVmuxThemeName(settingValue)) {
...
export function getAutoSidebarProjectColorTheme(
  ...
  const usedThemes = new Set<VmuxThemeName>();
