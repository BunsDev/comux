...
export async function launchPopup(
  ...
  const resultFile = path.join(os.tmpdir(), `vmux-popup-${Date.now()}.json`);
...
export function launchNodePopupNonBlocking<T = any>(
  ...
  const readyFile = path.join(
    os.tmpdir(),
    `vmux-popup-ready-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`
  );
  ...
  const envAssignments = [`VMUX_POPUP_READY_FILE='${escapedReadyFile}'`];
  if (themeName) {
    const escapedThemeName = themeName
      .replace(/\\/g, '\\\\')
      .replace(/'/g, "'\\''");
    envAssignments.push(`VMUX_THEME='${escapedThemeName}'`);
  }
