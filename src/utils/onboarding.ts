...
export async function runOpenRouterApiKeyOnboardingIfNeeded(): Promise<void> {
  ...
  try {
    ...
    if (!apiKey) {
      console.log(chalk.hex(COLORS.warning)('Skipping OpenRouter setup (no API key entered).'));
      await writeOpenRouterOnboardingState(homeDir, 'skip');
      return;
    }
    ...
    console.log(chalk.hex(COLORS.success)(`Saved OPENROUTER_API_KEY to ${shellConfigPath}`));
    console.log(chalk.hex(COLORS.warning)(`Run 'source ${shellConfigPath}' or open a new shell to load it globally.`));
  } catch (error) {
    logger.warn(
