...
import { buildRemotePaneActionCommand } from './vmuxCommand.js';

export const VMUX_CONTROLLER_PID_OPTION = '@vmux_controller_pid';
export const VMUX_CONTROL_PANE_OPTION = '@vmux_control_pane';
export const VMUX_REMOTE_PANE_ACTION_TABLE = 'vmux-pane-action';
export const VMUX_REMOTE_PANE_MODE_OPTION = '@vmux_remote_pane_mode';
...
const REMOTE_TITLE_RENAME_TRIGGER_BINDINGS = [
  { key: 'DoubleClick1Pane', noPrefix: true },
] as const;
...
const PANE_TITLE_MOUSE_CONDITION = '#{||:#{==:#{mouse_y},#{pane_top}},#{==:#{mouse_y},#{-:#{pane_top},1}}}';
const DEFAULT_DOUBLE_CLICK_PANE_COMMAND = [
  'select-pane -t =',
  'if-shell -F "#{||:#{pane_in_mode},#{mouse_any_flag}}" "send-keys -M" "copy-mode -H \\; send-keys -X select-word \\; run-shell -d 0.3 \\; send-keys -X copy-pipe-and-cancel"',
].join(' \\; ');
...
function buildRunRemotePaneActionCommandArgs(
  shortcut: RemotePaneActionShortcut
): string[] {
  return [
    'run-shell',
    `${buildRemotePaneActionCommand(shortcut)} >/dev/null 2>&1`,
  ];
}
...
function buildSafeTmuxCommandArgs(command: string): string[] {
  return [
    'run-shell',
    `tmux ${command} >/dev/null 2>&1 || true`,
  ];
...
function buildPaneTitleRenameCommand(): string {
  return `select-pane -t = \\; ${buildRunRemotePaneActionCommand('e')}`;
}
...
export function buildRemotePaneActionBindingCommands(): string[] {
  const menuCommands = REMOTE_MENU_TRIGGER_BINDINGS.map(({ key, noPrefix }) =>
    noPrefix
      ? `bind-key -n ${key} ${buildRunRemotePaneActionCommand('m')}`
      : `bind-key ${key} ${buildRunRemotePaneActionCommand('m')}`
  );

  const renameCommands = REMOTE_TITLE_RENAME_TRIGGER_BINDINGS.map(({ key, noPrefix }) => {
    const titleCommand = escapeForDoubleQuotes(buildPaneTitleRenameCommand());
    const fallbackCommand = escapeForDoubleQuotes(DEFAULT_DOUBLE_CLICK_PANE_COMMAND);
    return noPrefix
      ? `bind-key -n ${key} if-shell -F "${PANE_TITLE_MOUSE_CONDITION}" "${titleCommand}" "${fallbackCommand}"`
      : `bind-key ${key} if-shell -F "${PANE_TITLE_MOUSE_CONDITION}" "${titleCommand}" "${fallbackCommand}"`;
  });

  return [...menuCommands, ...renameCommands];
}

export function buildRemotePaneActionBindingCommandArgs(): string[][] {
  const menuCommands = REMOTE_MENU_TRIGGER_BINDINGS.map(({ key, noPrefix }) =>
    noPrefix
      ? ['bind-key', '-n', key, ...buildRunRemotePaneActionCommandArgs('m')]
      : ['bind-key', key, ...buildRunRemotePaneActionCommandArgs('m')]
  );
  ...
  const renameCommands = REMOTE_TITLE_RENAME_TRIGGER_BINDINGS.map(({ key, noPrefix }) =>
    noPrefix
      ? [
        ...
        'if-shell',
        '-F',
        PANE_TITLE_MOUSE_CONDITION,
        buildPaneTitleRenameCommand(),
        DEFAULT_DOUBLE_CLICK_PANE_COMMAND,
      ]
      : [
        'bind-key',
        key,
...
export function buildRemotePaneActionCleanupCommands(): string[] {
  const commands = [
    ...
    ...REMOTE_TITLE_RENAME_TRIGGER_BINDINGS,
  ...
  commands.push(
    buildSafeTmuxCommand(
      `unbind-key -T ${VMUX_REMOTE_PANE_ACTION_TABLE} Escape`
    ),
    buildSafeTmuxCommand(
      `unbind-key -T ${VMUX_REMOTE_PANE_ACTION_TABLE} C-c`
    ),
    buildSafeTmuxCommand(
      `unbind-key -T ${VMUX_REMOTE_PANE_ACTION_TABLE} Any`
    )
  );
  ...
  for (const shortcut of REMOTE_PANE_ACTION_SHORTCUTS) {
    commands.push(
      buildSafeTmuxCommand(
        `unbind-key -T ${VMUX_REMOTE_PANE_ACTION_TABLE} ${shortcut}`
      )
    );
  }
...
export function buildRemotePaneActionCleanupCommandArgs(): string[][] {
  const commands = [
    ...REMOTE_MENU_TRIGGER_BINDINGS,
    ...REMOTE_TITLE_RENAME_TRIGGER_BINDINGS,
    ...LEGACY_REMOTE_TRIGGER_BINDINGS,
