...
const ComuxApp: React.FC<ComuxAppProps> = ({
  ...
  const [showStartupPrimer, setShowStartupPrimer] = useState(false)
  ...
  // Bridge daemon pair banner state
  const [pairBanner, setPairBanner] = useState<PairBannerState | null>(null)
  ...
  useEffect(() => {
    ...
    void hasCompletedStartupPrimer()
      .then((completed) => {
        if (!cancelled) {
          setShowStartupPrimer(!completed)
        }
      })
      .catch(() => undefined)
  ...
  const completeStartupPrimer = useCallback((outcome: StartupPrimerOutcome) => {
    setShowStartupPrimer(false)
    void writeStartupPrimerState(outcome).catch(() => undefined)
  }, [])
  ...
  const createTerminalPaneForRitual = async (
    ...
    try {
      ...
      setStatusMessage(`Failed to create ritual terminal: ${error?.message || String(error)}`)
      setTimeout(() => setStatusMessage(""), STATUS_MESSAGE_DURATION_SHORT)
      return null
    } finally {
  ...
  const openRitual = async (
    ...
    activeProjectRoot: string = selectedProjectRoot
  ) => {
    ...
    for (const ritualProject of ritual.projects) {
      ...
      for (const ritualPane of ritualProject.panes) {
        ...
        const prompt = ritualPane.prompt?.trim()
          || `Start ${ritualPane.name || "this ritual pane"} for ${ritual.name}.`
        const availableForProject = getAvailableAgentsForProject(targetProjectRoot)
        const selectedAgents = ritualPane.agent && availableForProject.includes(ritualPane.agent)
          ? [ritualPane.agent]
          : await selectAgentsForPaneCreation(targetProjectRoot)

        if (selectedAgents === null) {
          setStatusMessage(`Cancelled ritual: ${ritual.name}`)
          setTimeout(() => setStatusMessage(""), STATUS_MESSAGE_DURATION_SHORT)
          return
        }
        ...
        if (createdPanes.length > 0) {
          ...
          createdCount += createdPanes.length
        }
      }
    }
  ...
  // Keep a stable ref to openRitual so the bridge launcher registration does
  // not need to re-run every render (openRitual captures many state values and
  // is not memoized with useCallback).
  const openRitualRef = useRef(openRitual)
  useEffect(() => {
    openRitualRef.current = openRitual
  })

  // Register the live ritual launcher with the bridge daemon once React has
  // mounted and openRitual is available. The daemon will call this in response
  // to a remote launchRitual RPC from iOS. The params map is ignored in v1 —
  // no ritual templates take params yet.
  useEffect(() => {
    if (!bridgeDaemon) return;
  ...
  useInputHandling({
    ...
    isUpdating,
    isLoading,
    ignoreInput: ignoreInput || showHooksPrompt || !!pairBanner,
    isDevMode,
    quitConfirmMode,
    setQuitConfirmMode,
    showCommandPrompt,
