 ...
 *
 * Instead of polling every 5 seconds, tmux hooks notify vmux immediately
...
export class TmuxHookManager extends EventEmitter {
  ...
  async areHooksInstalled(): Promise<boolean> {
    ...
    try {
      ...
      await execAsync(
        `tmux show-hooks -t '${this.sessionName}' 2>/dev/null | grep -q 'vmux-hook'`,
  ...
  async installHooks(): Promise<boolean> {
    ...
    try {
      ...
      const hookCommands = [
        ...
        `tmux set-hook -t '${this.sessionName}' after-split-window 'run-shell "kill -USR2 ${this.pid} 2>/dev/null || true # vmux-hook"'`,
        ...
        `tmux set-hook -t '${this.sessionName}' client-resized 'run-shell "kill -USR2 ${this.pid} 2>/dev/null || true # vmux-hook"'`,
        // Pane focus changed
  ...
  /**
   * Remove all vmux hooks from this session
   */
