/**
 * Centralized Logging Service for vmux
 *
...
/**
 * LogService singleton - central logging hub for vmux
 *
...
export class LogService extends EventEmitter {
  ...
  generateTestLogs(count: number = 100): void {
    ...
    const messages = [
      ...
      'Project root: /Volumes/StudioExternal/Primary/projects/vmux',
      'vmux started for project: vmux',
    ...
    for (let i = 0; i < count; i++) {
      ...
      const paneId = Math.random() > 0.7 ? `vmux-${Math.floor(Math.random() * 5)}` : undefined;
