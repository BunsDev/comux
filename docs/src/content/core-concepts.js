...
export function render() {
  return `
    <h1>Core Concepts</h1>
    <p class="lead">comux is built around three core concepts: projects, panes, and worktrees. Understanding how they interact will help you get the most out of parallel development.</p>

    <h2>Projects</h2>
    <p>A <strong>project</strong> in comux corresponds to a git repository. When you run <code>comux</code> inside a repo, it creates a stable, project-scoped tmux session named like <code>comux-your-project-a1b2c3d4</code>.</p>
    <ul>
      <li>Each project gets its own isolated tmux session</li>
      <li>Session state is stored in <code>.comux/comux.config.json</code></li>
    ...
    <ul>
      <li>A unique ID (e.g. <code>comux-1</code>, <code>comux-2</code>)</li>
    ...
    <ol>
      ...
      <li><strong>Monitor</strong> — comux tracks agent status (working, analyzing, waiting, idle)</li>
    ...
    <p>comux monitors each pane to detect the agent's state:</p>
    <table>
      <thead>
      ...
      <tbody>
        ...
        <tr><td><code>analyzing</code></td><td>comux is analyzing recent terminal output</td></tr>
    ...
    <p>When a pane settles into a state that needs you, comux marks it with an attention indicator in the sidebar and pane border. On macOS, comux can also send a native notification for panes that are not currently focused.</p>
    <ul>
      ...
      <li>On non-macOS platforms, comux still works normally without the native helper</li>
    </ul>

    <h3>Pane Visibility</h3>
    <p>comux lets you temporarily remove panes from the active window without stopping them.</p>
    <ul>
    ...
    <ul>
      ...
      <li>Preview either file contents or git diff output without leaving comux</li>
...
└── .comux/worktrees/
    ...
    <p>When you merge a pane (press <kbd>m</kbd> to open the menu, then select merge), comux performs a two-phase merge:</p>
    <ol>
    ...
    <div class="callout callout-info">
      ...
      If there are merge conflicts in step 2, comux will abort the merge and let you know which files conflict. You can resolve them manually in the worktree and retry.
    </div>

    <h2>The Sidebar</h2>
    <p>The comux TUI shows a sidebar with all active panes. Each pane displays:</p>
    <ul>
    ...
    <p>You can optionally use a sidebar layout mode (press <kbd>L</kbd>) where the comux sidebar occupies a fixed 40-column pane on the left, with agent panes arranged in a grid to the right.</p>
